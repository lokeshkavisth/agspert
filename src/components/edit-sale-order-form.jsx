...
const EditSaleOrderForm = ({ closeModal, saleData }) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      invoice_date: saleData?.invoice_date.slice("T")[0],
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      axios.put("http://localhost:5001/api/sales", values),
    onError: (error) => {
      console.error("mutation error", error);
    },
    onSuccess: (data) => {
      // Store the data in the cache using queryClient
      console.log("edited query data", data?.data);
      queryClient.setQueryData(["sale"], data?.data);
      reset();
      closeModal();
      navigate("/completed-sales", { replace: true });
    },
  });
...
export default EditSaleOrderForm;
